...
public class ShellSort {
  ...
  /**
   * 算法设计思路:
   * 1. 选择一个增量序列 (gap序列), 这里采用经典的 n/2, n/4, ..., 1
   * 2. 按增量序列个数 k, 对数组进行 k 趟排序
   * 3. 每趟排序, 根据当前增量 gap, 将数组逻辑上分成若干间隔为 gap 的子数组
   * (如 gap=3时, 子数组为 [0,3,6,...], [1,4,7,...], [2,5,8,...])
   * 4. 对每个子数组执行带有步长gap的插入排序
   * 5. 随着gap逐渐减小, 子数组元素越来越密集, 数组逐渐接近有序
   * 6. 当gap=1时, 执行标准插入排序, 此时数组已基本有序, 排序效率极高
   */

  /**
   * 算法复杂度分析:
   * - 时间复杂度: O(n^(1.3-2)), 其中 n 是数组长度.
   * 具体复杂度取决于增量序列的选择, 本实现采用经典序列, 平均情况约为 O(n^(1.5))
   * - 最好情况: O(n log n) (当数组已基本有序时)
   * - 最坏情况: O(n*n) (当增量序列选择不佳时)
   * - 空间复杂度: O(1).
   ...
   * 对整数数组进行希尔排序
   *
   * @param arr 待排序的整数数组 (支持null和空数组, 直接返回原数组)
   * @return 排序后的整数数组 (返回原数组引用, 非新数组)
   */
  public int[] sort(int[] arr) {
    // 边界情况处理: 空数组或只有一个元素的数组无需排序
