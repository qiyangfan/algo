...
/**
 * 测试类: Algo024MinDepthBFS
 * 使用 JUnit 5 测试使用广度优先搜索计算二叉树最小深度算法的正确性
 *
 * 注意: 本测试类与 Algo023 的测试用例完全相同, 因为两个算法解决同一个问题,
 * 只是实现方式不同(DFS vs BFS), 应该产生相同的结果.
 */
class Algo024MinDepthBFSTest {
    ...
    void testNullTree() {
        Algo024MinDepthBFS algo = new Algo024MinDepthBFS();
